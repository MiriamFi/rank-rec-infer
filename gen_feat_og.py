...
def add_age_feature(user_info, user_features):
    ...
    for u_id in range(len(user_info)):
        age = user_info[u_id]["age"]

        if age not in ages.keys():
            ages[age] = "age_" + str(len(ages))
    ...
    for u_id in range(len(user_info)):
        ...
        for a in ages:
            age_label = ages[a]
            user_features[u_id][age_label] = 0
        
        user_age = user_info[u_id]["age"]

        age_label = ages[user_age]
        user_features[u_id][age_label] = 1
