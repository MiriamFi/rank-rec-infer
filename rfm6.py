...
def cross_validate(clf, X_r1, X_r2, y_r1, y_r2, attr):
    ...
    for train_ix, test_ix in cv_outer.split(X_r1):
        ...
        #auc_score = get_roc_auc( y_test, y_prob, clf_classes)
        #auc_score = plot_roc_auc(y_test, y_prob, clf, attr)
        #print("auc: ", auc_score)
        

        # report progress
        print('>acc=%.3f, f1=%.3f, logl=%.3f, est=%.3f, cfg=%s' % (acc, f1, logl, result.best_score_, result.best_params_), "\n")
...
def get_roc_auc(y_test, y_prob, classes):
    
    clf_roc_auc = roc_auc_score(y_test, y_prob, average="macro", multi_class='ovr', labels=classes)
    return clf_roc_auc
