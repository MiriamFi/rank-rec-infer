...
RAN_FOR_HPARAMS = {
    ...
    "ran_for__max_features": [10],
    'ran_for__min_samples_leaf': [1],
    'ran_for__min_samples_split': [2]
}
...
def prepare_rec_splits(data, train_size=0.7, test_size=0.3):   
    ...
    ranks = data.groupby('user_id')['book_rating'].rank(method='first')
    #print("ranks: ", ranks)
    counts = data['user_id'].map(data.groupby('user_id')['book_rating'].apply(len))
    #print("counts: ", counts)
    ...
    X_train = X_train.drop(columns=['thrs_train', 'book_rating'], axis=1)
    # print("X_train: ", X_train)
    if train_size + test_size == 1.0:
        X_test = data[data['thrs_train'] == False]
        # print("X_test: ", X_test)
        X_test = X_test.drop(columns=['thrs_train', 'book_rating'], axis=1)
        # print("X_test: ", X_test)
    else:
        ...
        # print("X_test: ", X_test)
        X_test = X_test.drop(columns=['thrs', 'thrs_train', 'thrs_test', 'book_rating'], axis=1)
        # print("X_test: ", X_test)
...
def cross_validate(clf, X_r1, X_r2, y_r1, y_r2, attr):
    ...
    #for train_ix, test_ix in cv_outer.split(X_r1):
        ...
        X_train = X_r1[X_r1.index.isin(train_ix)]
        X_test = X_r2[X_r2.index.isin(test_ix)]
        ...
        if INFER_ATTR[attr] == True:
            ...
            for clf in CLASSIFIERS.keys():
                if CLASSIFIERS[clf] == True:
                    ...
                    result['clf'] = clf
                    result['attr'] = attr
                    (result['acc'], result['f1'], result['auc']) = cross_validate(clf, rec_train, rec_test, attributes_train[attr], attributes_test[attr], attr)
