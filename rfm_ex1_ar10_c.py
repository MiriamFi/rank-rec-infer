...
def write_clf_scores_to_csv(all_results, metric):
    ...
    with open(filename, 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter=' ', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        csv_writer.writerow(
            ["Clf\t", "Gender\t\t\t", "Age\t\t\t", "Job\t\t\t", "Area_5\t\t\t", "Area_2\t\t\t", "Area_10\t\t\t"])
        ...
        with open(filename, 'a', newline='') as csvfile:
            ...
            csv_writer.writerow(
                [clf, output[clf]["gender"], output[clf]["age"], output[clf]["occupation"], output[clf]["area_5"],
                 output[clf]["area_2"], output[clf]["area_10"]])
...
def main():
    ...
    for attr in INFER_ATTR.keys():
        if INFER_ATTR[attr] == True:
            ...
            attributes_train[attr] = prepare_attributes_for_classifier(user_info, users, attr_type=attr)
            attributes_test[attr] = prepare_attributes_for_classifier(user_info, users, attr_type=attr)
