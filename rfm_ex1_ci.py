...
def plot_roc_auc(y_test, y_prob, clf, attr):
    ...
    #plt.show()
    filename = "output_ex1_ci/plots/auc_"  + attr + "_" + clf
...
def write_double_rec_to_csv(rec_train, scores_train, rec_test, scores_test):
    ...
    if N > 20:
        ...
        with open('output_ex1_ci/recomended_items.csv', 'w', newline='') as csvfile:
        ...
        for usr in range(rec_train.shape[0]):
            for rnk in range(rec_train.shape[1]):
                ...
                with open('output_ex1_ci/recomended_items.csv', 'a', newline='') as csvfile:
                    csv_writer = csv.writer(csvfile, delimiter='\t', quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    csv_writer.writerow(
...
def write_clf_preds_to_csv(result):
    ...
    base = "output_ex1_ci/prediction_values/clf_pred"
